...
router.post("/transaction", async (req, res) => {
  const { address, cost, email, mobile, productQuantity } = req.body;  /// pass transaction data
  console.log(address, cost, email, mobile, productQuantity);



  const user = await User.findOne({ email: email });   // Find user bank ID
  console.log(`asd= ${user.name} ${user?.money}??na`);
  if (user.money < cost) {
    return res
      .status(301)
